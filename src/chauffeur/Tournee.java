...
public class Tournee {
	...
	private int nbCPs = 0;
	public String[] cPs = new String[nbCodePostauxMax];
	...
	public String caracteristiquesTournee() {
		return chauffeur.getNom() + " " + chauffeur.getPrenom() + " conduit : " + camion.getNumeroIm()
				+ " pour la tournée : " + getNom();
	}
	
	public boolean codePostalDedans(String codePostal) {

		for (int i = 0; i < this.getNbCodePostaux(); i++) {
			if (codePostal.equals(this.getcPs()[i])) {
				return true; 
			}
		}
		return false; 
